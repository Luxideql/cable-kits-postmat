...
//   A: id
//   B: Назва матеріалу
//   C: Одиниця виміру (шт / м / кг / ...)
//   D: Кількість на 1 комплект (норма витрати основного матеріалу)
//   E: Альтернативна позиція (назва замінника, якщо основний закінчився)
//   F: Альт. кількість на 1 компл. (норма витрати альтернативного матеріалу)
//   G: Запас основний (поточний залишок основного матеріалу)
//   H: Фактичний залишок складу — осн. (ручний підрахунок для звірки)
//   I: Запас альтернативний (поточний залишок альтернативного матеріалу)
//   J: Факт. залишок складу — альт. (ручний підрахунок для звірки)
//   K: Примітка
//   L: Активний (true / false — для видалення без втрати рядка)
...
export async function addMaterial(m: Omit<Material, 'id'>): Promise<{ id: string }> {
  await sheetEnsure('Матеріали', MATERIAL_HEADERS);
  const id = genId('mat');
  await sheetAppend('Матеріали!A:N', [
...
export async function deleteMaterial(id: string): Promise<void> {
  ...
  const col = activeColIdx >= 0 ? String.fromCharCode(65 + activeColIdx) : 'L';
  await sheetUpdate(`Матеріали!${col}${idx + 1}`, [['false']]);
}
